...
import { calculateCompletion as clientCalculateCompletion } from './data-client';
...
const readData = (): AppData => {
  ...
  try {
      ...
      console.error("Failed to parse db.json, returning initial data", e);
      return initialMockData;
  }
};
...
export const calculateCompletion = (items: { completed: boolean }[] | { silos: { tasks: { completed: boolean }[] }[] } | { projects: { silos: { tasks: { completed: boolean }[] }[] }[] }) => {
    return clientCalculateCompletion(items);
};
